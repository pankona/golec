...
* A のブランチで作業 → プルリク提出
* プルリクのレビューを待っている間の作業は、A をベースにして B のブランチを作成して作業。
* A のプルリクに修正が入った場合、B のブランチに A の変更を反映させる必要がある。
  * B のブランチにて、`git rebase A` をやれば OK。

### コミットログは適当でいい？

* 諸説あるが、赤塚は適当ではダメだと思う派。
  * コミットログはあとから変更の理由を探るのに役立つ。
  * レビュー時においても変更の意図を把握するのに役立つ。
  * 関連チケットの番号と、変更の意図を書く。what や how ではなく、why が大事。
    * how はコードの変更を見れば分かるが、何故その変更が必要になったという「理由」は往々にしてコードには表れない。
  * コミットログのタイトル部分は、"If applied, this commit will" に続けてしっくりくるように書くと良いとか。
...
* コミットログの書き方参考
  * [Gitコミットメッセージの7大原則 - rochefort's blog](http://rochefort.hatenablog.com/entry/2015/09/05/090000)
  * [Gitコミットメッセージのプラクティスまとめ - 酒と涙とRubyとRailsと](http://morizyun.github.io/blog/git-commit-log-format/index.html)
...
* 後々 grep でひっかけられるか、将来的に見たときに意味が分かるか (意図が思い出せるか) などを考えて書く。
